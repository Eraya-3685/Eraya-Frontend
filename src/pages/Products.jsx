...
const Products = () => {
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Header */}
      <div className="border-b border-slate-100 py-8 bg-slate-50/10">
        <div className="max-w-screen-2xl mx-auto px-4 md:px-8">
          <div className="flex justify-between items-end">
            <div>
              <h1 className="text-2xl font-black text-slate-900 tracking-tight">Eraya Collection</h1>
              <p className="text-slate-400 text-[10px] font-black mt-1 uppercase tracking-widest">{total} pieces available</p>
            </div>
      ...
      <div className="max-w-screen-2xl mx-auto px-4 md:px-8 py-12 flex flex-col lg:flex-row gap-12">
              ...
              <AnimatePresence mode="popLayout">
                {products.map((product, i) => (
                  ...
                  >
                    ...
                    <div className="relative aspect-[4/5] rounded-[32px] overflow-hidden bg-slate-50/50 mb-5 transition-all duration-700 group-hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.15)] group-hover:-translate-y-2">
                      ...
                      <div className="absolute top-5 left-5 right-5 flex justify-between items-start">
                        {product.discount_price && product.base_price > product.discount_price ? (
                          <div className="bg-red-500 text-white px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-lg shadow-red-500/20">
                            -{Math.round(((product.base_price - product.discount_price) / product.base_price) * 100)}%
                          </div>
                        ) : <div />}
                        
                        <button className="bg-white/80 backdrop-blur-md p-2 rounded-xl text-slate-400 hover:text-red-500 hover:bg-white transition-all shadow-sm">
                           <Heart className="w-3.5 h-3.5" />
                        </button>
                      </div>

                      {/* Interaction Actions */}
                      <div className="absolute bottom-6 left-6 right-6 flex gap-2 translate-y-12 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-700">
                        {user?.role !== 'admin' && (
                          <button
                            onClick={() => handleAddToCart(product)}
                            className="flex-grow bg-slate-900 text-white h-12 rounded-2xl flex items-center justify-center gap-2 font-black text-[9px] uppercase tracking-widest hover:bg-primary transition-all shadow-2xl active:scale-95"
                          >
                        ...
                        <Link
                          ...
                          className="w-12 h-12 bg-white text-slate-900 rounded-2xl flex items-center justify-center shadow-2xl hover:bg-primary hover:text-white transition-all"
                        >
                    ...
                    <div className="px-1 space-y-2">
                      <div className="flex justify-between items-start gap-4">
                        <Link to={`/products/${product.slug}`} className="flex-grow">
                          <h3 className="font-black text-slate-900 group-hover:text-primary transition-colors text-[15px] leading-snug tracking-tight line-clamp-2">
                            {product.name}
                          </h3>
                        </Link>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-baseline gap-2">
                           <span className="text-lg font-black text-slate-900 tracking-tighter">৳{product.base_price}</span>
                           {product.discount_price && (
                             <span className="text-slate-300 line-through text-[11px] font-bold opacity-60">৳{product.discount_price}</span>
                           )}
                        </div>
